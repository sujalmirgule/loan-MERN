...
import { Activity, Search, RotateCcw, Eye, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
...
export const AdminActiveLoansPage: React.FC = () => {
  ...
  const { data: response, isLoading, isFetching, refetch } = useQuery({
    queryKey: ['adminActiveLoans', page, search, stateFilter, loanTypeFilter],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), pageSize: '25', status: 'APPROVED', paymentStatus: 'PAID', ...(search ? { search } : {}), ...(stateFilter ? { state: stateFilter } : {}), ...(loanTypeFilter ? { loanType: loanTypeFilter } : {}) });
  ...
  const loans: ActiveLoan[] = Array.isArray(raw?.data) ? raw.data : Array.isArray(raw?.data?.data) ? raw.data.data : [];
  ...
  const handleReset = () => { setSearch(''); setSearchInput(''); setStateFilter(''); setLoanTypeFilter(''); setPage(1); };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            <Activity className="w-6 h-6 text-blue-600" /> Active Loans
          </h1>
          <p className="text-sm text-text-secondary mt-0.5">All active approved loans in the live portfolio</p>
        </div>
      ...
      <Card className="shadow-sm">
        <CardContent className="p-4 space-y-3">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1"><Search className="w-4 h-4 text-text-secondary absolute left-3 top-2.5" /><Input placeholder="Search name, mobile, account #..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="pl-9 text-sm" /></div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-3 border-b flex-row items-center justify-between">
          <CardTitle className="text-sm">Active Loans ({pagination.total})</CardTitle>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="py-16 text-center"><Activity className="w-10 h-10 text-text-secondary mx-auto mb-2" /><p className="text-sm text-text-secondary">No active loans found</p></div>
          ) : (
            <>
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-xs text-left">
                  <thead className="bg-slate-50 border-b text-text-secondary font-semibold uppercase tracking-wider">
                    <tr>
                      <th className="py-3 px-4">Account #</th><th className="py-3 px-4">Customer</th><th className="py-3 px-4">State</th>
                      <th className="py-3 px-4">Loan Type</th><th className="py-3 px-4 text-right">Sanctioned</th>
                      <th className="py-3 px-4 text-right">EMI</th><th className="py-3 px-4">Tenure</th>
                      <th className="py-3 px-4">Next EMI</th><th className="py-3 px-4 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {loans.map((loan) => (
                      <tr key={loan.id} className="hover:bg-blue-50/20 transition-colors">
                        <td className="py-3 px-4 font-mono font-semibold text-slate-700">{loan.accountNumber || loan.applicationNumber}</td>
                        ...
                        <td className="py-3 px-4 text-right font-bold text-blue-700">{fmtCurr(loan.approvedAmount || loan.requestedAmount)}</td>
                        ...
                        <td className="py-3 px-4 text-slate-600">{loan.nextEmiDate ? new Date(loan.nextEmiDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) : '—'}</td>
              ...
              <div className="md:hidden p-3 space-y-3">
                {loans.map((loan) => (
                  <div key={loan.id} className="border border-blue-200 rounded-xl p-3.5 bg-blue-50/20 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs font-bold text-slate-700">{loan.accountNumber || loan.applicationNumber}</span>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div><span className="text-text-secondary block">Sanctioned</span><span className="font-bold text-blue-700">{fmtCurr(loan.approvedAmount || loan.requestedAmount)}</span></div>
