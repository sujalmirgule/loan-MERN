...
import {
  CheckCircle2, Search, RotateCcw, Eye, FileText, ChevronLeft, ChevronRight, Loader2,
} from 'lucide-react';
...
interface ApprovedLoan {
  ...
  estimatedEmi?: number;
  status: string;
  customer: { id: string; fullName: string; mobile: string; state?: string; city?: string };
}

export const AdminApprovedLoansPage: React.FC = () => {
  ...
  const { data: response, isLoading, isFetching, refetch } = useQuery({
    queryKey: ['adminApprovedLoans', page, search, stateFilter, loanTypeFilter, dateFrom, dateTo],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), pageSize: '25', status: 'APPROVED', ...(search ? { search } : {}), ...(stateFilter ? { state: stateFilter } : {}), ...(loanTypeFilter ? { loanType: loanTypeFilter } : {}), ...(dateFrom ? { dateFrom } : {}), ...(dateTo ? { dateTo } : {}) });
  ...
  const loans: ApprovedLoan[] = Array.isArray(raw?.data) ? raw.data : Array.isArray(raw?.data?.data) ? raw.data.data : [];
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            <CheckCircle2 className="w-6 h-6 text-emerald-600" /> Approved Loans
          </h1>
          <p className="text-sm text-text-secondary mt-0.5">All sanctioned loan applications</p>
        </div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-3 border-b flex-row items-center justify-between">
          <CardTitle className="text-sm">Approved Loans ({pagination.total})</CardTitle>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="py-16 text-center"><CheckCircle2 className="w-10 h-10 text-text-secondary mx-auto mb-2" /><p className="text-sm text-text-secondary">No approved loans found</p></div>
          ) : (
            <>
                ...
                <table className="w-full text-xs text-left">
                  <thead className="bg-slate-50 border-b text-text-secondary font-semibold uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="py-3 px-4">Loan Type</th><th className="py-3 px-4 text-right">Approved Amt</th>
                      <th className="py-3 px-4 text-right">EMI</th><th className="py-3 px-4">Tenure</th><th className="py-3 px-4 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {loans.map((loan) => (
                      <tr key={loan.id} className="hover:bg-emerald-50/20 transition-colors">
                        ...
                        <td className="py-3 px-4 text-right font-bold text-emerald-700">{fmtCurr(loan.approvedAmount || loan.requestedAmount)}</td>
                        <td className="py-3 px-4 text-right text-slate-700">{loan.finalEmi || loan.estimatedEmi ? fmtCurr(loan.finalEmi || loan.estimatedEmi || 0) : '—'}</td>
                        <td className="py-3 px-4 text-slate-600">{loan.tenureMonths} Mo.</td>
                        <td className="py-3 px-4">
                          <div className="flex items-center justify-end gap-1.5">
                            <Button size="sm" variant="outline" onClick={() => navigate(`/admin/loans/${loan.id}`)} className="h-7 px-2 text-[11px] border-slate-300">
                              <Eye className="w-3 h-3 mr-1" />
                              View
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => navigate(`/approval-letter/${loan.id}`)} className="h-7 px-2 text-[11px] border-emerald-300 text-emerald-700 bg-emerald-50/50 hover:bg-emerald-100">
                              <FileText className="w-3 h-3 mr-1" />
                              Letter
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => navigate(`/admin/loans/${loan.id}`)} className="h-7 px-2 text-[11px] border-blue-300 text-blue-700 bg-blue-50/50 hover:bg-blue-100">
                              Agreement
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => navigate(`/admin/loans/${loan.id}`)} className="h-7 px-2 text-[11px] border-slate-300 text-slate-700">
                              EMI
                            </Button>
                          </div>
                        </td>
              ...
              <div className="md:hidden p-3 space-y-3">
                {loans.map((loan) => (
                  <div key={loan.id} className="border border-emerald-200 rounded-xl p-3.5 bg-emerald-50/20 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs font-bold text-slate-700">{loan.applicationNumber}</span>
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800">Approved</span>
                    </div>
                    <p className="font-semibold text-sm">{loan.customer.fullName} · {loan.customer.state}</p>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div><span className="text-text-secondary block">Approved</span><span className="font-bold text-emerald-700">{fmtCurr(loan.approvedAmount || loan.requestedAmount)}</span></div>
                      <div><span className="text-text-secondary block">EMI</span>{loan.finalEmi ? fmtCurr(loan.finalEmi) : '—'}</div>
                    </div>
